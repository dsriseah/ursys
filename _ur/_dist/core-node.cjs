...
// node-server/@node-index.mts
var node_index_exports = {};
__export(node_index_exports, {
  ADDON: () => ur_addon_mgr_exports,
  APPBUILD: () => appbuilder_exports,
  APPSERV: () => appserver_exports,
  ASSETSERV: () => assetserver_exports,
  CLASS: () => common_classes_exports,
  DATA_QUERY: () => util_data_search_exports,
  DATA_UTIL: () => util_data_ops_exports,
  FILE: () => file_exports,
  NET_UTIL: () => util_urnet_exports,
  NORM: () => util_data_norm_exports,
  PROC: () => process_exports,
  PROMPTS: () => util_prompts_exports,
  SNA: () => sna_node_exports,
  TEXT: () => util_text_exports,
  TerminalLog: () => makeTerminalOut,
  UID: () => module_uid_exports
});
module.exports = __toCommonJS(node_index_exports);

// node-server/sna-node.mts
...
// node-server/sna-node-urnet-server.mts
var import_node_path4 = __toESM(require("node:path"), 1);

// node-server/file.mts
var file_exports = {};
...
var u_path = (p = "") => {
  ...
  p = import_node_path.default.normalize(import_node_path.default.join(ROOT, p));
...
function FindParentDir(rootFile, startDir) {
  const fileUrl = import_meta.url || `file://${process.cwd()}`;
  let currentDir = startDir || url.fileURLToPath(new URL(".", fileUrl));
  const u_check_dir = (dir) => FSE.existsSync(import_node_path.default.join(dir, rootFile));
...
function DetectedAddonDir(aoName) {
  ...
  const adir = import_node_path.default.join(root, "_ur_addons");
  const cwd = process.cwd();
  if (!cwd.includes(adir)) {
    if (aoName === void 0)
      throw Error(`${fn2}: autodetect fail; use ${fn2}('addon-name') syntax`);
    if (!DirExists(import_node_path.default.join(adir, aoName)))
      throw Error(`${fn2}: addon '${aoName}' not found in ${adir}`);
    return [aoName, import_node_path.default.join(adir, aoName)];
  }
  const addon = cwd.slice(adir.length + 1).split(import_node_path.default.sep)[0];
  return [addon, import_node_path.default.join(adir, addon)];
}
...
function GetDirContent(dirpath, opt = { absolute: true }) {
  ...
  for (let name of filenames) {
    let path2 = import_node_path.default.join(dirpath, name);
...
function Files(dirpath, opt = { absolute: false }) {
  ...
  if (opt.absolute) return result.files.map((p) => import_node_path.default.join(dirpath, p));
...
function TrimPath(p = "") {
  p = import_node_path.default.join(p);
  if (p.startsWith("/")) p = p.slice(1);
  if (p.endsWith("/")) p = p.slice(0, -1);
  return p;
}
function GetPathInfo(path2) {
  const bn = import_node_path.default.basename(path2, import_node_path.default.extname(path2));
  const en = import_node_path.default.extname(path2).slice(1);
  const dn = import_node_path.default.dirname(path2);
  return {
    isDir: en.length === 0,
    isFile: en.length > 0,
    filename: `${bn}.${en}`,
...
var import_serve_index = __toESM(require("serve-index"), 1);
...
function ListenHTTP(opt) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (index_file === void 0) APP.get("/", (0, import_serve_index.default)(http_docs));
    else {
    ...
    APP.use(import_express.default.static(http_docs));
...
var import_esbuild_plugin_copy = require("esbuild-plugin-copy");
var SRC_JS;
var SRC_ASSETS;
var PUBLIC;
var RUNTIME;
...
async function BuildApp(opts) {
  ...
  const context = await import_esbuild.default.context({
    ...
    plugins: [
      ...
      (0, import_esbuild_plugin_copy.copy)({
        resolveFrom: "cwd",
        assets: [
          {
            from: [`${SRC_ASSETS}/**/*`],
...
async function WatchExtra(opts) {
  ...
  const watcher = import_chokidar.default.watch(watch_dirs, {
    persistent: true
  });
...
// node-server/util-dynamic-import.mts
...
async function FindServerModules(absSrcDir) {
  ...
  try {
    ...
    const mtsFiles = (await import_node_fs.default.promises.readdir(absSrcDir)).filter(mtsFilter);
    for (const file of mtsFiles) {
      const absFile = import_node_path3.default.join(absSrcDir, file);
...
async function FindClientEntryFiles(srcDir) {
  ...
  try {
    ...
    const clientFiles = (await import_node_fs.default.promises.readdir(srcDir)).filter(tsFilter);
    return clientFiles;
  } catch (error) {
    throw Error(`${fn2} Error during dynamic import: ${error.message}`);
  }
...
`;
    if (clientFiles.length === 0) {
      const shortDir = import_node_path3.default.basename(srcDir);
...
`;
    ...
    const outPath = import_node_path3.default.join(srcDir, outFile);
    await import_node_fs.default.promises.writeFile(outPath, out);
...
`;
    ...
    const id = import_node_path3.default.basename(srcDir);
...
// node-server/sna-node-context.mts
...
function SNA_EnsureAppDirs(rootDir) {
  const source_dir = import_node_path4.default.join(rootDir, "app-source");
  const asset_dir = import_node_path4.default.join(rootDir, "app-static");
  const output_dir = import_node_path4.default.join(rootDir, "_public");
  const runtime_dir = import_node_path4.default.join(rootDir, "_runtime");
  const config_dir = import_node_path4.default.join(rootDir, "_config");
  const viewlib_dir = import_node_path4.default.join(source_dir, "viewlib");
  const webc_dir = import_node_path4.default.join(viewlib_dir, "webc");
...
// node-server/sna-node-hooks.mts
...
// node-server/sna-dataserver.mts
...
// node-server/sna-dataobj-adapter.mts
...
var FSE2 = __toESM(require("fs-extra"), 1);
var import_path = __toESM(require("path"), 1);
var import_express2 = __toESM(require("express"), 1);
var import_serve_index2 = __toESM(require("serve-index"), 1);

// node-server/util-express.mts
...
async function DeliverManifest(req, res, next) {
  ...
  const path2 = import_path.default.join(m_asset_path, pathname);
...
function SetupServer() {
  ...
  FSE2.ensureDirSync(import_path.default.join(assetPath));
  const INDEX_TEXT = `${PACKAGE_NAME} - GEMSTEP MAIN CONTROL PROGRAM HOST `;
  const INDEX_SEND = import_path.default.join(assetPath, "_serverId.txt");
  ...
  APP2.use(
    ...
    import_express2.default.static(assetPath),
    // UR.MediaProxy_Middleware({}) // asset host servers do not proxy
    (0, import_serve_index2.default)(assetPath, { "icons": true })
  );
}

// node-server/process.mts
var process_exports = {};
...
function DecodeAddonArgs(argv) {
  ...
  const addonScript = import_node_path5.default.basename(a_enp, import_node_path5.default.extname(a_enp));
  return [
    addonScript,
...
function ValidateAddon(addon) {
  const ADDONS = import_node_path5.default.join(DetectedRootDir(), "_ur_addons");
  if (!DirExists(ADDONS)) {
  ...
  const addon_dir = import_node_path5.default.join(ADDONS, addonName);
  ...
  if (!entryName) {
    if (entryFiles.length > 0) {
      ...
      entryName = import_node_path5.default.basename(entryFile, import_node_path5.default.extname(entryFile));
      return {
        addonName,
        entryName,
        entryFile,
...
// node-server/class-urmodule.mts
...
// common/declare-errors.ts
var EXIT_CODES = {
  ERR_UR: 444
  // ur runner general error
};

// common/util-error-mgr.ts
...
function m_ReadModuleEntryFiles(modname) {
  ...
  const modulePath = (0, import_node_path6.join)(URDIR, modname);
...
//# sourceMappingURL=core-node.cjs.map
