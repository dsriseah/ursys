...
module.exports = __toCommonJS(sna_node_exports);
...
async function FilesHashInfo(filepaths, algo = "md5") {
  ...
  for (let fp of filepaths) {
    ...
    const { filename, basename: basename2, ext } = GetPathInfo(fp);
    hashInfo.push({ filepath: fp, filename, basename: basename2, ext, hash });
  }
  return hashInfo;
}
...
async function m_GetDataBinEntries(dataPath, assetPath) {
  ...
  for (let info of filesInfo) {
    const { filename, basename: basename2, ext, hash } = info;
...
//# sourceMappingURL=sna-node.cjs.map
