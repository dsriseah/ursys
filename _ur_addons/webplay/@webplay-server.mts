...
async function m_ImportServerModules(): Promise<string[]> {
  try {
    ...
    const mtsFiles = files.filter(file => file.endsWith('.mts'));
    for (const file of mtsFiles) await import(`./scripts/${file}`);
    return mtsFiles;
  } catch (error) {
...
async function m_ImportClientModules(): Promise<string[]> {
  try {
    ...
    for (const file of tsFiles) out += `import './scripts/${file}';\n`;
...
(async () => {
  ...
  LOG(`${BLU}QUICKSTART: ts and mts files are autoloaded from './scripts'${NRM}`);
